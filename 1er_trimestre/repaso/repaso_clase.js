...
//Es mejor poner entre comillas("") lo de nombre, edad y profesión
let personas=[{
    nombre: 'Ana',
    edad: 19,
...
function MasEdad(personas,num){
    ...
    for(let cada_persona of personas){
        if(cada_persona.edad>num){
            console.log(cada_persona)//añadir final
        }
    }
    return 
}
...
function NombresPorProfesion(personas,profesiones){
    for(let cada_profesion of profesiones){
        ...
        console.log(cada_profesion+':')
        for(let cada_personas of personas){
            if(cada_personas.profesion==cada_profesion){
                console.log(cada_personas.nombre)
                encontrados=true

            }
        }
        if(encontrados==false){
            console.log('Nadie en esta profesión')
        }
    }
    return
}
NombresPorProfesion(personas,profesiones)
